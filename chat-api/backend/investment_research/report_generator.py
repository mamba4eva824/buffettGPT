...
class ReportGenerator:
    ...
    4. Caches reports in DynamoDB for fast retrieval

    Modes:
    - API mode: Set ANTHROPIC_API_KEY for automated generation
    - Claude Code mode: Use prepare_data() to get metrics, then save_report() to cache
    ...
    def __init__(self, use_api: bool = False, prompt_version: float = 4.8):
        ...
        table_name = os.environ.get('INVESTMENT_REPORTS_TABLE', 'investment-reports-dev')
    ...
    def save_report(
        ...
        """
        Save a report generated by Claude Code to DynamoDB.

        Replaces any existing reports for this ticker to ensure only 1 version
        exists per ticker. This prevents stale reports from accumulating.

        Args:
            ticker: Stock symbol
            fiscal_year: Fiscal year
            report_content: Full markdown report content
            ratings: Structured ratings dict with debt, cashflow, growth ratings
            features: Optional features snapshot to store
        """
        ticker = ticker.upper()
        print(f"  Saving report for {ticker} (prompt v{self.prompt_version}) to DynamoDB...")

        # Delete any existing reports for this ticker (ensures only 1 version)
        self._delete_existing_reports(ticker)

        # Serialize ratings to JSON string (DynamoDB doesn't handle floats in maps)
        # This also makes it consistent with features_snapshot format
        ratings_json = json.dumps(ratings, cls=DecimalEncoder)

        item = {
            'ticker': ticker,
            'fiscal_year': fiscal_year,
            'report_content': report_content,
            'ratings': ratings_json,  # Store as JSON string for DynamoDB compatibility
            'generated_at': datetime.utcnow().isoformat() + 'Z',
            'model': 'claude-code',  # Generated via Claude Code
            'prompt_version': str(self.prompt_version),  # Store as string for DynamoDB compatibility
        ...
        print(f"  Report saved for {ticker} (prompt v{self.prompt_version})")

    def _delete_existing_reports(self, ticker: str) -> int:
        """
        Delete all existing reports for a ticker.

        Used to ensure only 1 version of a report exists per ticker.
        ...
        # Query all reports for this ticker
        ...
        if deleted_count > 0:
            print(f"  Deleted {deleted_count} existing report(s) for {ticker}")
    ...
    async def generate_report(
        ...
        if not self.use_api:
            raise ValueError(
                ...
                "Use prepare_data() and save_report() for Claude Code mode."
            )
    ...
    def _format_metrics_for_prompt(
        ...
        for year in sorted_years:
            ...
            output.append(f"| {year} | {fmt.money(rev)} | {fmt.money(ni)} | {gm}% | {nm}% | {fmt.eps(eps)} |")
        ...
        for year in sorted_years:
            ...
            output.append(f"| {year} | {fmt.money(ocf)} | {fmt.money(fcf)} | {fcf_margin}% | {fmt.money(capex)} | {fmt.money(dividends)} | {fmt.money(buybacks)} | {ocf_ni}x |")

        # Balance Sheet Trend Table (Enhanced)
        # Note: "Liquidity" includes cash + short-term investments for accurate fortress assessment
        output.append("\n### Balance Sheet Trend")
        output.append("| Year | Total Debt | Liquidity* | Net Debt | Equity | Debt/Equity | Debt/Assets | Current Ratio |")
        output.append("|------|------------|------------|----------|--------|-------------|-------------|---------------|")
        for year in sorted_years:
            data = annual_data.get(year, {})
            bs = data.get('balance', {})
    ...
    def _cache_report(
        ...
        Stores the full report with TTL for automatic expiration (4 months).
        """
        # Convert features to JSON string (DynamoDB doesn't support nested floats well)
        features_json = json.dumps(decimal_to_float(features), cls=DecimalEncoder)
    ...
    def _get_cached_report(self, ticker: str, fiscal_year: int) -> Optional[dict]:
        ...
        Returns None if no cached report or if TTL has passed.
        """
        try:
            ...
            if item:
                # Check if TTL has passed (DynamoDB TTL deletion is not instant)
                ttl = item.get('ttl', 0)
                if ttl > datetime.utcnow().timestamp():
                    return item
            return None
        except Exception as e:
